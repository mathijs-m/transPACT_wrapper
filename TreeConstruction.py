...
# The DistanceTreeConstructor.upgma method is changed to use numpy arrays instead of Python loops.
# This leads to a speedup of several orders of magnitude for large matrices.
...
class DistanceTreeConstructor(TreeConstructor):
    ...
    def upgma(self, distance_matrix):
        ...
        min_j = 0
        inner_count = 0
        # In the standard implementation of DistanceMatrix object, the matrix is a standard python list of lists. This has poor performance.
        # Hence, convert the triangular matrix to a np array first
        np_array = np.ones([len(dm), len(dm)])*np.infty
        for row_index, row in enumerate(dm.matrix):
            np_array[row_index][:len(row)] = row
        # We're also not considering the diagonal
        np.fill_diagonal(np_array, np.infty)
        dm.matrix = np_array

        #sys.stdout.write('Started minsearch.\n')
        while dm.matrix.shape[1] > 1:
            t = time.time()
            min_dist = np.min(dm.matrix[1:,:-1])
            flattened_min_indices = np.flatnonzero(dm.matrix == min_dist)
            # Obtain the i and j indices from the flattened array
            min_i, min_j = np.unravel_index(flattened_min_indices[-1], dm.matrix.shape)

            sys.stdout.write("=======================\nMin found after %.4f sec.\n==================" % (time.time()-t))
            ...
            for k in range(0, len(dm.matrix)):
                if k != min_i and k != min_j:
                    dm.matrix[k, min_j] = (dm.matrix[k, min_j] + dm.matrix[k, min_j]) * 1.0 / 2

            dm.names[min_j] = "Inner" + str(inner_count)

            # Remove the processes leaves from the matrix
            dm.matrix = np.delete(np.delete(dm.matrix, (min_i), axis=0), (min_i), axis=1)
